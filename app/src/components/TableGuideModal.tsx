...
function Box({ x, y, w, h, r = 4, strong = false, dashed = false }: { x: number; y: number; w: number; h: number; r?: number; strong?: boolean; dashed?: boolean }) {
  return (
    <rect
      x={x}
      y={y}
...
function Line({ x, y, w, light = false }: { x: number; y: number; w: number; light?: boolean }) {
  return <rect x={x} y={y} width={w} height={5} rx={2.5} className={light ? 'fill-neutral-700' : 'fill-neutral-600'} />
}

function Label({ x, y, children, size = 10, anchor = 'start', muted = false }: { x: number; y: number; children: React.ReactNode; size?: number; anchor?: 'start' | 'middle' | 'end'; muted?: boolean }) {
  return (
...
function Chip({ x, y, w, color, text }: { x: number; y: number; w: number; color: string; text?: string }) {
  return (
    <g>
...
function Highlight({ x, y, w, h, r = 5 }: { x: number; y: number; w: number; h: number; r?: number }) {
